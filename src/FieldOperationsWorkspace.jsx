        ...
        {fieldOperationsTab === "dailyLog" ? (
          ...
          <>
            <Section
              ...
              right={<div className="actionRow"><button type="button" className="secondaryButton" disabled={fieldLogsLoading} onClick={refreshFieldLogs}>Refresh logs</button><button type="button" className="secondaryButton" disabled={fieldLogsLoading || Boolean(fieldLogsError)} onClick={handleExportPayrollCsv}>Export Payroll CSV</button></div>}
            >
              <p className="smallNote">{isFinanceUser ? "Company-wide submitted logs and your own drafts. Other employees’ drafts stay private." : "Your own logs. Submitted crew logs are available to office administrators and finance."}</p>
              {fieldLogsLoading ? <p role="status">Refreshing daily logs…</p> : null}
              {fieldLogsError ? <p role="alert">Daily logs could not be refreshed: {fieldLogsError}</p> : null}
              <div className="formGrid">
                <Field label="Search">
