...
import { TASK_FILE_OPTIONS, INSPECTION_FILE_OPTIONS, TASK_ATTACHMENT_ACCEPT, createTaskUpload, saveTaskAttachment, validateTaskAttachment } from './taskAttachments.js';
export default function TaskAttachments({ supabase, taskId, userId, attachmentType = "task", readOnly = false, onPendingChange }) {
  const options = attachmentType === "inspection" ? INSPECTION_FILE_OPTIONS : TASK_FILE_OPTIONS;
  const [items, setItems] = useState([]), [pending, setPending] = useState([]);
  const [busy, setBusy] = useState(false), [error, setError] = useState(''), [notice, setNotice] = useState('');
  const locked = useRef(false);
  const refresh = useCallback(async () => {
    const { data, error } = await supabase.from(options.table).select('*').eq(options.foreignKey, taskId).order('created_at');
    if (error) throw error; setItems(data || []);
  }, [supabase, taskId, options]);
  ...
  useEffect(() => { onPendingChange?.(pending.length > 0 || busy); return () => onPendingChange?.(false); }, [pending.length, busy, onPendingChange]);
  const upload = async (entries) => {
    ...
    try {
      for (const entry of entries) {
        try { await saveTaskAttachment(supabase, taskId, entry, options); saved++; }
  ...
  const open = async item => {
    setError('');
    try {
      const { data, error } = await supabase.storage.from(options.bucket).createSignedUrl(item.storage_path, 600, { download: item.file_name });
  ...
  return <section className="taskAttachments" aria-label={attachmentType === "inspection" ? "Inspection attachments" : "Task attachments"}>
    <h4>Photos &amp; attachments</h4>
    {!readOnly ? <FileDropZone accept={TASK_ATTACHMENT_ACCEPT} label="Add Photos & Files" help={attachmentType === "inspection" ? "Photos and documents, up to 25 MB each. Draft files stay private until submission." : "Photos, PDFs, Word, Excel, CSV or text. Up to 25 MB per file. Only this task’s participants can open them."} disabled={busy} onFiles={choose} /> : null}
