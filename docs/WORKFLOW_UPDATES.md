...
## Inspection Request Center and request attachments — September 25, 2026

- Added a dedicated Inspection Request Center in navigation and Tasks & Messages, with private drafts, assigned-technician/office queues, search/status filters, appointments, findings, measurements, and readable audit history. Workflow statuses include New, Contacted, Scheduled, Inspected, Proposal Requested, On Hold, and Cancelled. Earlier accessible inspection tasks remain available without rewriting them; future submissions through the existing CRM route enter the center automatically.
- Photos and documents can be selected before submission or added to an open request. Save Draft & Attachments persists a private request and uploads its files; failed uploads block submission and retain retry IDs. Registered files use private storage and signed downloads. Draft text persists on the device; unsaved file selections must be reselected after closing/reloading. Files are limited to 25 MB and the supported image/document formats. Abandoned unregistered private uploads may need administrative cleanup.
- Confirming an inspection requires findings. Proposal handoff requires findings, measurements and an address, reuses the existing Daniela review gate, and exposes the same inspection attachments in the proposal detail without copying them or inventing pricing. Optimistic version checks prevent silent overwrites; retries do not duplicate tasks, proposals or attachment audit entries. Submitted reassignment is office-only and does not expand the original task discussion's participants.
- Applied and recorded migration `20260925160000_inspection_request_center.sql` after successful production ROLLBACK validation. It adds three RLS-protected inspection tables, guarded RPCs, audit/CRM intake triggers, and a private attachment bucket. Existing business records were not modified and no synthetic live request/file was created.
- Deployed `process-workflow-notifications` with first-contact reminders after 24 hours, upcoming appointment reminders within 24 hours, and passed-appointment follow-up. These route to the assigned technician and Jorge/Natalia, exclude drafts/held/closed/inspected requests, and deduplicate daily. The previously deployed salesperson follow-up remains three days after a proposal is sent.
- Validation: 327 automated tests passed, including executable Postgres access isolation, draft privacy, storage registration, guarded transitions, idempotent submission, and the actual existing proposal-handoff function. Production build and targeted lint passed; the build retains an existing large-chunk warning. An isolated mobile browser flow exercised selecting an attachment before submit, simulated upload failure and retry, inspection confirmation, proposal handoff, linked evidence display, and opening a fresh proposal form. Live write acceptance, native phone photo/camera behavior, and recipient inbox delivery remain manual checks with genuine records.
- Live verification caught that production `user_profiles` has no `is_active` column. Applied follow-up migration `20260925161000_inspection_profile_compatibility.sql`, preserving the existing provisioned-profile/role model, and removed that nonexistent field from the team query. The Postgres fixture now omits it to match production; all 327 tests, the focused lifecycle test, build, and targeted lint pass. Scheduled live scans returned HTTP 200 with `scanFailed: false`; the new private bucket and migration record were verified with zero inspection rows at activation.
